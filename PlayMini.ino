...
void playSound10To90(DFPlayerMini player, int num, int delayTimeMs = 0)
{
  switch (num) {
    case 10:
...
void playSoundBlackBox(DFPlayerMini player, String text, int delayTimeMs)
{
  ...
  if (text.length() == 1) {
    ...
    num = int(text.substring(0, 2).toInt());
    Serial.println("20");
    playSound10To90(player, int(num / 10) * 10, delayTimeMs);
       Serial.println("DEpois 20");
